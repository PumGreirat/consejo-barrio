...
import { Profile, Report, emptyReportData, getOrg, ROLE_LABELS } from '@/types'
...
export default function ReportsTab({ profile, reports, onRefresh }: Props) {
  ...
  const myReports = reports.filter(r => r.org_id === org.id).sort((a, b) =>
  ...
  async function deleteReport(rep: Report, e: React.MouseEvent) {
    e.stopPropagation()
    const msg = rep.status === 'published'
      ? `¿Eliminar este reporte?\n\nEste reporte está publicado. Al eliminarlo desaparecerá de "Mis Reportes", pero los asuntos que enviaste al Consejo seguirán apareciendo en la Vista del Consejo hasta que el obispado los elimine.`
    ...
    <div>
      <div className="card">
        <div className="flex items-start justify-between mb-5 gap-3 flex-wrap">
          ...
          <span className="org-badge px-3.5 py-1.5 rounded-full text-xs font-bold"
            style={{ background: org.color + '18', color: org.color, border: `1.5px solid ${org.color}40` }}>
            {org.name}
          </span>
        </div>

        <button onClick={() => setShowNewModal(true)} className="btn bn mb-5">
        ...
        {myReports.length === 0 ? (
          ...
          <div className="space-y-2.5">
            {myReports.map(r => {
              ...
              return (
                <div key={r.id} className="flex items-center gap-4 p-4 rounded-xl border-[1.5px] border-[#ddd6c8] bg-white hover:border-gold hover:bg-amber-50/30 transition-all">
                  <button className="flex items-center gap-4 flex-1 min-w-0 text-left" onClick={() => setActiveReport(r)}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">
                        {sundayStr ? `Consejo: ${sundayStr}` : dateStr} · {totalItems} items · {updStr}
                      </p>
                    </div>
                    <span className={`sec-pill ${pub ? 'status-published' : 'status-draft'} flex-shrink-0`}>
          ...
          <div className="bg-white rounded-2xl p-8 w-full max-w-md shadow-2xl">
            ...
            {newDate && (
              <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-5">
                📅 Este reporte se asignará automáticamente al domingo <strong>{format(new Date(getNextSunday(new Date(newDate + 'T12:00:00')) + 'T12:00:00'), "d 'de' MMMM", { locale: es })}</strong>
              </p>
            )}
            <div className="flex gap-2.5">
              <button className="btn bn" onClick={createReport} disabled={creating || !newName.trim()}>
                {creating ? 'Creando...' : 'Crear reporte'}
              </button>
              <button className="btn bg" onClick={() => { setShowNewModal(false); setNewName('') }}>Cancelar</button>
            </div>
          </div>
        </div>
      )}
